...
df["xyz_order"] = df["ECEF"].replace(xyz_order)
...
sat["r_itrf"] = np.array(
    [
        df[df["ECEF"] == "x"]["position"],
        df[df["ECEF"] == "y"]["position"],
        df[df["ECEF"] == "z"]["position"],
    ]
)
sat["v_itrf"] = (
    np.array(
        [
            df[df["ECEF"] == "x"]["velocity"],
            df[df["ECEF"] == "y"]["velocity"],
            df[df["ECEF"] == "z"]["velocity"],
        ]
    )
    / 1e4
)
...
for i in range(N):
    sat["r_itrf"][:, i] = (
        np.linalg.norm(sat["r_itrf"][:, i]) - (c * clock_bias[i])
    ) * at.normalize(sat["r_itrf"][:, i])
    if i > 0:
        sat["v_itrf"][:, i] = sat["v_itrf"][:, i] - c * clock_drift[
            i - 1
        ] * at.normalize(sat["r_itrf"][:, i])
...
EOP2 = at.parseEOPFile(
    "./astrotools/EOP2long.txt"
)  # inport Earth Orientation Parameters for polar motion rotation
...
for k in range(N - 1):
    ...
    f = lambda t, x: at.orbit_ode(
        t,
        x,
        earth.mu,
        gravity="J",
        max_degree=max_degree,
        Re=earth.SemimajorAxis,
        C=earth.C,
        S=earth.S,
        GMST=GMST[k],
        J=earth.J,
...
print(
    "Maneuvers detected by velocity residuals at time steps:", maneuver_indices_velocity
)
...
for k in range(N_prop - 1):
    ...
    _, x_prop[:, k + 1] = at.rk4_substeps(
        f, 0, x_prop[:, k], dt_prop[k], num_substeps=30
    )
